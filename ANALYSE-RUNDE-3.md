...
| **R3-D** ✅ **ERLEDIGT (v2.14.0)** | Nachhol-Pass Domänen 5+6 (init/config_flow/schema/config_helpers + HA-Konformität/Deprecations/Test-Fidelity/CI-Meta) → 16 neue Befunde R3D-01..R3D-16 (s. §6). Deprecation-Sweep sauber. 13 umgesetzt, 3 begründet zurückgestellt. | Audit-Rest |

---

## 6. R3-D — Nachhol-Audit Domänen 5+6 (v2.14.0)

**Methode:** 2 Finder-Agenten (Domäne 5 = init/config_flow/schema/config_helpers; Domäne 6 = HA-Konformität/Deprecations/Test-Fidelity/CI-Meta), jeder Befund empirisch gegen die installierte HA 2026.7.2 verifiziert, gegen den bekannten Backlog (R3-01..26, §6-A-Backlog, ANALYSE-UND-ROADMAP-Wellen) dedupliziert; anschließend selbst verifiziert + adversarial reviewt.
**Kernergebnis:** Der **Deprecation-Sweep ist SAUBER** — kein von der Integration genutzter HA-Import ist auf 2026.7.2 veraltet/entfernt (moderne APIs durchgehend: `async_forward_entry_setups`, `async_migrate_entry`+`er.async_migrate_entries`, `@callback`, frozen `EntityDescription`, `UnitOf*`/`SensorDeviceClass`/`ColorMode`; keine `SUPPORT_*`/`DEVICE_CLASS_*`/`hass.helpers.*`). translations/strings-Parität ok, manifest korrekt.

### Domäne 5 (Produktcode)
- **R3D-01 · MEDIUM · config_flow.py · ✅ v2.14.0** — Config-Flow lehnte für LAN-Gateways Hostnamen/`.local` ab (`ipaddress.ip_address`-only), obwohl die Laufzeit Namen auflöst → LAN-Gateway per Name nicht per UI hinzufügbar. Fix: bei Nicht-IP jetzt `getaddrinfo` (im Executor); auflösbarer Host → ok. **Review-Nachbesserung:** `getaddrinfo` wirft für fehlerhafte Namen (führender Punkt, `..`, Label >63) `UnicodeError` (KEIN `OSError`) → nun `except (OSError, UnicodeError)` (sonst Config-Flow-Absturz statt sauberer Ablehnung); zusätzlich leere/blanke Adresse vorab abgelehnt (`getaddrinfo("")` löst auf Loopback auf). Beide regressionsgetestet.
- **R3D-02 · MEDIUM(latent) · schema.py/init · ✅ v2.14.0** — Schema-Default `port=5100` überschrieb den VNG-Fallback `VIRT_GW_PORT` → VNG ohne explizites `port:` band 5100 statt 12345. Fix: Schema-Default entfernt (init liefert den Typ-Default).
- **R3D-03 · LOW · schema.py · ✅ v2.14.0** — `reconnection_timeout`/`tcp_keep_alive_timeout` akzeptierten `0` (`cv.positive_float` inkl. 0) → Flatter/Spin. Fix: `Range(min=1)`.
- **R3D-04 · LOW · init · ✅ v2.14.0** — VNG (LAN-Typ) verlangte fälschlich `address:`, das es nie nutzt. Fix: Adresspflicht für `VirtualNetworkAdapter` übersprungen.
- **R3D-05 · LOW · schema.py · ✅ v2.14.0** — totes `_get_receiver_schema` (nie referenziert; hätte `gateway_id` unter `sender:` als Extra-Key die ganze Config abgelehnt). Fix: entfernt.
- **R3D-06 · LOW · config_helpers.py · ✅ v2.14.0** — `get_general_settings_from_configuration`-Guard schützte nur `hass is None`, nicht den `[DATA_ELTAKO][ELTAKO_CONFIG]`-Index (KeyError, wenn ungesetzt; heute nicht erreichbar). Fix: `.get()`-Verkettung.
- **R3D-07 · LOW · config_helpers.py · ✅ v2.14.0** — `get_id_from_gateway_name` nahm den ERSTEN `(Id: N)`-Treffer → falsche id, wenn der Nutzer-Name selbst `(Id: N)` enthält. Fix: Regex an das nachgestellte `(Id: N)$` verankert.
- **R3D-16 · LOW · schema.py · ✅ v2.14.0** (Cross-Check) — `min_target_temperature >= max_target_temperature` wurde still akzeptiert (invertierter Sollwertbereich). Fix: `vol.All`-Validator lehnt min≥max ab.

### Domäne 6 (Deprecations sauber; Test-Fidelity/CI/Meta)
- **R3D-09 · MEDIUM · tests/test_gateway.py · ✅ v2.14.0** — modulweite Monkeypatches beim Import (nie zurückgesetzt) → order-/selektionsabhängige Suite; `asyncio.get_event_loop = Mock(None)` setzte `HassMock.loop=None` (latente Falle), `RS485SerialInterface`-Mock traf die falsche Klasse (Prod nutzt V2). Fix: in setUpModule/tearDownModule gescopt; die zwei überflüssigen Patches entfernt.
- **R3D-11 · LOW-MED · tests/test_send_message_service.py · ✅ v2.14.0** — schrieb bei jedem Lauf die git-getrackte `docs/service-send-message/eep-params.md` (dirty tree; CWD-abhängig). Fix: Schreiben in Temp-Datei + Content-Assertion (das „revert vor Commit" entfällt).
- **R3D-13 · LOW · requirements.txt · ✅ v2.14.0** — tote Einträge `StrEnum` (seit R3-26 stdlib) und `termcolor` (ungenutzt). Fix: entfernt.
- **R3D-15 · LOW · .DS_Store · ✅ v2.14.0** — getrackte macOS-Artefaktdatei in `custom_components/eltako/`. Fix: `git rm --cached` + `.gitignore`.
- **R3D-08 · LOW · gateway.py · ✅ v2.14.0** — tote `self._loop = asyncio.get_event_loop()` (nie gelesen; in Sync-Kontext unerwünscht). Fix: entfernt.
- **R3D-10 · MEDIUM · tests/test_tcp_connection_hardening.py + VNG-Tests · ⏸ ZURÜCKGESTELLT** — Real-Socket-Loopback-Tests sind nicht-hermetisch, verschlucken Server-Thread-Fehler (z. B. WinError 10013 in einem Daemon-Thread, während „OK" gemeldet wird) → Umgebungs-/Order-Flakiness + falsche Grün-Meldung. Fix (M): Thread-Exceptions einfangen/propagieren, deterministisch joinen oder `socketpair`-Fakes. **Zurückgestellt:** dediziertes Test-Hardening; Änderung an aktuell grünen Tests birgt eigenes Flakiness-Risiko.
- **R3D-12 · LOW-MED · .github/workflows + requirements.txt · ⏸ ZURÜCKGESTELLT** — CI nicht reproduzierbar: ungepinntes `homeassistant`, `python-version: '3.x'`, `setup-python@v4`, floatende Action-Refs (`@master`/`@main`), kein Coverage-Gate. Fix (S): HA/Python/Actions pinnen. **Zurückgestellt:** CI-Änderungen bergen Risiko für die grüne Release-Pipeline; separater, iterativer Task.
- **R3D-14 · LOW · services.yaml · ⏸ ZURÜCKGESTELLT** — leere `services.yaml` → der Sende-Service hat keine UI-Metadaten. Design-gekoppelt: der Servicename ist dynamisch pro Gateway (`eltako_gw_<id>_send_message`), statisch nicht beschreibbar. Fix (M, Design): fixer Servicename + `target`/Data-Selektoren. **Zurückgestellt:** Redesign.

**Nicht als Bug bestätigt (Domäne 6 explizit sauber):** Deprecation-Sweep (0 Warnungen auf 2026.7.2), `DeviceInfo`-Import-Pfad, manuelle `entity_id`-Slugs (gültig), `select.py self.name`-Zuweisung (cached_property-Shadow, kein Crash), translations-Parität, `hacs.json` (`name` genügt), Mock-Fidelity (FakeConfigEntries modelliert HAs Change-Detection).
