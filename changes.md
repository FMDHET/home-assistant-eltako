...
## Version 2.16.1 — LAN-Gateway: Schluss mit der `Replacing Packet.optional`-Flut im HA-Log
Ein LAN-Gateway (ZGW16-IP, MGW, EUL) schrieb dauerhaft `WARNING (Thread-3) [enocean.protocol.packet] Replacing Packet.optional with default value.` ins Home-Assistant-Log — bei Standardeinstellungen rund **2900 Zeilen pro Tag**. Die Meldung ist harmlos, aber sie macht das Log unbrauchbar und verdeckt echte Fehler. Ursache und Behebung liegen im **Sende-**, nicht im Empfangspfad.
* **Ursache:** `Packet.__init__` der `enocean`-Bibliothek warnt, sobald das Argument `optional` keine Liste ist — und sein eigener Default ist `None`. Jede Aufrufstelle, die das Argument weglässt, löst die Warnung also aus, unabhängig vom Telegramm. Der TCP-Keep-Alive von `esp2_gateway_adapter` baut genau so ein Paket: Nach `tcp_keep_alive_timeout` Sekunden Busstille (Standard 30 s) schickt er eine `CO_RD_IDBASE`-Anfrage als Lebenszeichen — und damit im Mittel etwa **eine Warnung pro Minute**. Dazu kamen zwei Zeilen pro (Neu-)Verbindung aus der Basis-ID- und Versionsabfrage.
* **Behebung:** Die drei ausgehenden `COMMON_COMMAND`-Pakete werden jetzt mit `optional=[]` gebaut. **Auf dem Draht ändert sich nichts** — für ein Common Command ist ein leeres Optional-Feld ohnehin der korrekte Wert, der erzeugte Frame ist Byte für Byte identisch (per Test abgesichert). Es verschwindet ausschließlich die Log-Zeile.
* **Kein Zusammenhang mit „Maybe have wrong device id configured!":** Diese parallel auftretende Warnung stammt aus einer rein statischen Konfigurationsprüfung, die einmalig beim Setup läuft und keine empfangenen Pakete anfasst. Sie ist ein eigenes Thema und von diesem Fix unberührt.
* **Nicht** über `logger:`-Konfiguration stummgeschaltet: Das hätte die Ursache stehen lassen und zusätzlich die durchaus nützlichen Warnungen `Replacing sender with default address.` und `Replacing destination with broadcast address.` mit unterdrückt.
* Umgesetzt als Override in `tcp2serial_hardened.py` (Marker `K9`) — die Bibliotheken unter `site-packages/` bleiben unangetastet und updatefest. Sinnvolle Ergänzung wäre ein Upstream-Issue bei `esp2_gateway_adapter`; danach kann der Override beim nächsten Anheben des Pins wieder entfallen.
* Tests: 5 neue Tests (`test_tcp_connection_hardening.py`), inkl. Nachweis, dass der Frame unverändert bleibt und dass der Abbruch-Zweig des kopierten Keep-Alive weiterhin greift; Suite **329 grün**.
...
* **Hintergrund/Nutzen:** Ein Aktor, der über einen F6-Rocker als *Sender* geschaltet wird, ist „fire-and-forget" und liefert keine Statusrückmeldung — die Entität bleibt dann auf `unknown` und Home Assistant zeigt den Zustand nicht sauber an (statt eines eindeutigen Toggles). Mit `fast_status_change: True` für genau dieses Gerät hält die Entität nach jedem Schaltbefehl einen definierten `on`/`off`-Zustand.
* **Definierter Startzustand:** Ist `fast_status_change` für ein Gerät aktiv und beim Start kein eindeutiger Zustand wiederherstellbar (Erstinstallation oder zuvor `unknown`), startet die Entität mit `off` statt `unknown` — so ist von Anfang an ein eindeutiger Zustand da. Ein echt wiederhergestellter `on`/`off`-Zustand wird dabei **nie** überschrieben.
* Tests: 18 neue Tests (`test_fast_status_change_per_device.py`); Suite **322 grün**. Adversarial reviewt (2 Finder): die Pro-Gerät-Verdrahtung ist sauber und rückwärtskompatibel. **Hinweis:** Die genaue UI-Wirkung (einzelner Toggle statt getrennter Ein/Aus-Bedienelemente) hängt vom HA-Kartentyp ab und ist am echten Gerät zu verifizieren, sobald die Hardware wieder läuft — die Funktion selbst ist verhaltensneutral und opt-in.

## Version 2.14.0 — Audit-Runde 3, Welle R3-D: Config-Flow, Schema & Aufräumen
Letzte Umsetzungswelle aus `ANALYSE-RUNDE-3.md` — der Nachhol-Audit der Domänen 5+6 (Integrations-Setup / Config-Flow / Schema / Config-Helfer + HA-Konformität / Deprecation-Sweep / Test-Fidelity / CI-Meta), den der Original-Audit am Kostenlimit abgebrochen hatte. **Gute Nachricht vorweg: der Deprecation-Sweep ist sauber** — kein von der Integration genutzter Home-Assistant-Baustein ist auf HA 2026.7 veraltet oder entfernt. 16 neue Befunde gefunden, 13 umgesetzt, 3 bewusst zurückgestellt; adversarial reviewt.
* **LAN-Gateway per Hostname/`.local` hinzufügbar (R3D-01):** Der UI-Einrichtungsdialog akzeptierte für LAN-Gateways bisher nur eine IP-Adresse und lehnte Hostnamen ab — obwohl die Laufzeit Namen problemlos auflöst. Ein per Name (z. B. `eltako-mgw.local`, `fgw14.fritz.box`) adressiertes LAN-Gateway ließ sich damit gar nicht über die Oberfläche einrichten. Jetzt wird ein auflösbarer Hostname akzeptiert.
* **Virtual Network Gateway nutzt wieder den vorgesehenen Port (R3D-02):** Ein VNG ohne explizites `port:` band Port 5100 statt des vorgesehenen VNG-Ports (12345), weil ein Schema-Standardwert den VNG-Fallback überschrieb. Behoben.
* **VNG braucht kein `address:` mehr (R3D-04):** Ein VNG ließ sich ohne das (für ihn bedeutungslose) Feld `address:` nicht einrichten. Behoben.
* **Robustere Schema-Validierung:** Verbindungs-Timeouts akzeptieren keine `0` mehr (führte zu Verbindungs-Flattern; R3D-03); eine Climate-Konfiguration mit `min_target_temperature ≥ max_target_temperature` wird jetzt abgewiesen statt still übernommen (R3D-16).
* **Kleinere Korrektheits-/Aufräum-Punkte:** Gateway-ID-Erkennung robust, wenn der Gateway-*Name* selbst „(Id: N)" enthält (R3D-07); defensiv gehärteter Zugriff auf die General-Settings (R3D-06); totes Schema-Fragment und eine tote Codezeile entfernt (R3D-05, R3D-08).
* **Test- & Repo-Hygiene:** Der Test-Lauf verändert nicht mehr die versionierte `docs/.../eep-params.md` (schreibt in eine Temp-Datei; R3D-11); order-abhängige globale Test-Mocks in `test_gateway.py` sind jetzt sauber gescopt (R3D-09); tote Abhängigkeiten (`StrEnum`, `termcolor`) aus `requirements.txt` entfernt (R3D-13); eine versehentlich versionierte `.DS_Store` entfernt (R3D-15).
* **Bewusst zurückgestellt (dokumentiert in `ANALYSE-RUNDE-3.md` §6):** hermetische Umstellung der Socket-Loopback-Tests (R3D-10), CI-Reproduzierbarkeit/Pinning (R3D-12), `services.yaml`-Metadaten für den Sende-Service (R3D-14, benötigt Redesign des dynamischen Servicenamens).
* Tests: 10 neue Tests (`test_r3d_config.py`); Suite **304 grün**. Adversarial reviewt (Finder-Befund behoben: der Hostname-Fix R3D-01 fing `UnicodeError` aus `getaddrinfo` bei fehlerhaften Namen wie `.local` nicht — hätte den Config-Flow abstürzen lassen — und akzeptierte leere Adressen; beide korrigiert und regressionsgetestet).
